#' Jarvis stomatal conductance model. Described in 
#' https://doi.org/10.1098/rstb.1976.0035.
#'
#' @param rad Downwelling radiation or PPFD
#' @param ta Air temperature
#' @param vpd Vapor pressure deficit
#' @param a 3-vector of constants fit by optimization
#' @param th Lower temperature bound of transpiration
#' @param tl Upper temperature bound of transpiration
#' @param gsmax Maximum allowed stomatal conductance in the model
#'
#' @return
#' @export
#'
#' @examples
jarvis_gs_model <- function(a, rad, ta, vpd, th=40, tl=0,
                            gsmax=0.8) {
  a1 <- a[1]
  a2 <- a[2]
  a3 <- a[3]
  ...
  f1 <- (rad) / (rad + a1)
  f2_exp <- (th - a2) / (a2 - tl)
  f2 <- ((ta - tl) * (th - ta) ^ f2_exp) / ((a2 - tl) * (th - a2) ^ f2_exp)
  f3 <- exp(-a3 * vpd)
...
eb_constants <- list(
  ...
  l=40.66e3    # Heat of vaporization of water J/mol
)